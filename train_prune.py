...
from models.with_mobilenet import PoseEstimationWithMobileNet_new
...
def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size, batches_per_iter,
          ...
          val_labels, val_images_folder, val_output_name, checkpoint_after, val_after, LOG_DIR):
  
    LOG_FOUT = open(os.path.join(checkpoints_folder, 'log_train.txt'), 'w')
    

    checkpoint = torch.load(checkpoint_path)
    checkp_prune = torch.load('/cluster/home/u4/extractor_cpm/0.3.pth.tar')
    cfg = checkp_prune['cfg']
    net = PoseEstimationWithMobileNet_new(cfg, num_refinement_stages)
    ...
    for epochId in range(current_epoch, 280):
        ...
        for batch_data in train_loader:
            ...
            #    net.train()
        ###log_train
        LOG_FOUT.write('EPOCH: {}'.format(epochId)+'\n')
        LOG_FOUT.flush()
        num_iter_per_epooch = len(dataset)/float(batch_size)
        for loss_idx in range(len(total_losses) // 2):
            LOG_FOUT.write('\n'.join(['stage{}_pafs_loss:     {}', 'stage{}_heatmaps_loss: {}']).format(loss_idx + 1, total_losses[loss_idx * 2 + 1] / num_iter_per_epooch,loss_idx + 1, total_losses[loss_idx * 2] /num_iter_per_epooch )+'\n')
            LOG_FOUT.flush()

        ###save_model
        snapshot_name = '{}/checkpoint_epoch_{}.pth.tar'.format(checkpoints_folder, epochId)
        torch.save({'state_dict': net.module.state_dict(),
                            'optimizer': optimizer.state_dict(),
                            'scheduler': scheduler.state_dict(),
                            'iter': num_iter,
...
if __name__ == '__main__':
    ...
    train(args.prepared_train_labels, args.train_images_folder, args.num_refinement_stages, args.base_lr, args.batch_size,
          ...
          args.checkpoint_after, args.val_after, args.log_dir)
